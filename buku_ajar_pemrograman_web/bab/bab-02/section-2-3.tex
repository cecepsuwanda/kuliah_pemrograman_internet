...
Teknologi server-side pada mata kuliah ini berfokus pada PHP yang berjalan di server dan terintegrasi dengan MySQL \cite{php-manual}, \cite{mysql}. Kode server-side bersifat tertutup dari pengguna; server memproses permintaan, mengakses basis data, menerapkan logika bisnis, dan mengembalikan hasil. Data sensitif dan validasi penting harus dilakukan di server untuk keamanan \cite{php-right-way}.

Sebagai contoh, saat pengguna mengirim form login, browser menjalankan JavaScript untuk validasi format (client-side) dan mengirim data ke server. Server menjalankan PHP untuk memeriksa kredensial di basis data MySQL (server-side). Server mengembalikan respons (berhasil atau gagal) dan browser menampilkan hasilnya. Memahami peran masing-masing lapisan membantu pengembang merancang aplikasi yang aman dan efisien \cite{webdev}.
