...
\centering
\small
\begin{tabular}{|P{4.2cm}|P{3.8cm}|P{2.6cm}|}
\hline
\textbf{Operasi} & \textbf{Metode} & \textbf{Catatan} \\
\hline
Buat elemen & \raggedright\raggedright\code{document.}\linebreak[0]\code{createElement(tag)} & Belum ada di DOM \\
\hline
...
\hline
Sisip sebelum & \raggedright\raggedright\code{parent.}\linebreak[0]\code{insertBefore(el, ref)} & Sebelum elemen ref \\
\hline
Hapus elemen & \code{el.remove()} & Hapus dari DOM \\
\hline
Clone & \code{el.cloneNode\allowbreak(true)} & deep clone \\
\hline
\end{tabular}
