...
\begin{tikzpicture}[node distance=3cm, >=stealth]
  \node[client] (client) {\shortstack{Browser\\(Client)}};
  \node[server, right=of client] (server) {\shortstack{Web Server\\(Server)}};
...
\hline
Teknologi & HTML, CSS, JavaScript & PHP (XAMPP) \\
\hline
Peran utama & Menampilkan, berinteraksi & Memproses, menyimpan data \\
\hline
Akses data & Melalui request/form & Langsung ke basis data \\
\hline
\end{tabular}
