...
Gambar \ref{fig:peta-konsep} memperlihatkan peta konsep utama dalam pemrograman internet. Alur pembelajaran dimulai dari fondasi (arsitektur, HTTP), kemudian membangun antarmuka (HTML, CSS, JavaScript), lalu logika server dan basis data, serta diakhiri dengan integrasi aplikasi dan keamanan.
...
\begin{tikzpicture}[
  ...
  \node[box, below=of db] (api) {Integrasi \& Keamanan};
  \draw[->] (arch) -- (html);
  \draw[->] (arch) -- (css);
  \draw[->] (arch) -- (js);
  \draw[->] (html) -- (server);
