\documentclass[../main]{subfiles}
\ifSubfilesClassLoaded{\setcounter{chapter}{2}}{}
\begin{document}

\chapter{HTML5: Struktur dan Elemen Dasar}

\begin{subcpmk}
  \item Sub-CPMK 2.1: Membuat halaman web dengan HTML5 (struktur, form, semantic elements)
\end{subcpmk}

\input{bab-03/section-3-1}
\input{bab-03/section-3-2}

\begin{aktivitas}
  \item Buat halaman HTML5 dengan struktur semantik (\texttt{header}, \texttt{nav}, \texttt{main}, \texttt{footer}) untuk profil diri.
  \item Tambahkan link, gambar, dan tabel ke halaman tersebut.
\end{aktivitas}

\begin{latihan}
  \item Jelaskan peran \texttt{<!DOCTYPE html>} dan bagian \texttt{head} serta \texttt{body}!
  \item Sebutkan minimal 4 elemen semantik HTML5 dan kegunaannya!
\end{latihan}

\begin{checklist}
  \item Dapat membuat dokumen HTML5 minimal
  \item Dapat menggunakan elemen semantik, link, gambar, tabel
\end{checklist}

\begin{rangkuman}
Bab ini membahas struktur dasar HTML5, elemen semantik, link, gambar, dan tabel. HTML5 adalah fondasi struktur konten web \cite{mdn-learn-html}.
\end{rangkuman}

\ifSubfilesClassLoaded{
  \renewcommand{\bibname}{Daftar Pustaka}
  \bibliographystyle{plain}
  \bibliography{references}
}{}
\end{document}
